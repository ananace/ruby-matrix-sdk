...
module MatrixSdk::Protocols::MSC::MSC2108
  MatrixSdk::Protocols::MSC.add_impl self, 2108 do
  ...
  #   end
  #
  # @return [Thread, {run: true}] The background thread and a cancellation token for the SSE stream
  #
  ...
  def msc2108_sync_sse(since: nil, **params, &on_data)
    ...
    thread = Thread.new(cancellation_token) do |ctx|
      print_http(req)
      loc_http = http(threaded: true)
      loc_http.request req do |response|
